...
/**
 * Created by ZOROASTRO on 03/12/2015.
 */
...
public class XMLParserAhorcado {
	private URL strURL;
    private Context context;
    private String inputStream;
	
	public XMLParserAhorcado(String strURL, Context context) {
	...
	public ArrayList<DatosAhorcado> parse() {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        ArrayList<DatosAhorcado> ArrayAhorcado = new ArrayList<DatosAhorcado>();
        try
        {
            ...
            {
                DatosAhorcado palabra = new DatosAhorcado();

                //Obtenemos el palabra
                ...
                for (int j=0; j<datosNoticia.getLength(); j++)
                {
                    ...
                    switch (j){
                        ...
                        case 5:
                            ...
                            {
                                String texto = obtenerTexto(dato);
                                // palabra.setDescripcion(texto);
                                // palabra.setLongitud(texto);
                            }
                            break;
                        case 7:
                            if (etiqueta.equals("string"))
                            {
                                String texto = obtenerTexto(dato);
                                // palabra.setImagen(texto);
                                // palabra.setImgJuego(resizeImage(descargarImagen(texto),180,180));
                            }
                            break;
                    }
                }
                ArrayAhorcado.add(palabra);
            }
        }
        catch (Exception ex)
        {
